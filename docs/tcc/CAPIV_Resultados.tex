...
\hline
Cadastro básico & Cadastrar local, leitor RFID e item patrimonial com a tag disponível no experimento. & Registros disponíveis para consulta e uso nos fluxos de leitura. & Validado funcionalmente na aplicação web e pela persistência dos registros. \\
\hline
Leitura válida & Ativar janela de leitura e aproximar a tag cadastrada do leitor de proximidade. & Sistema registra a leitura e atualiza o local físico conforme o leitor. & Validado fisicamente com leitor USB, comunicador intermediário e envio do evento à API. \\
\hline
Tag desconhecida & Enviar código de tag não cadastrado por simulação controlada na API. & Sistema registra evento e sinaliza inconsistência de tag desconhecida. & Verificado por teste funcional de software; validação física depende de nova tag ou massa de testes ampliada. \\
\hline
Local divergente & Associar o item a local lógico diferente do local vinculado ao leitor e aproximar a tag cadastrada. & Sistema identifica divergência entre localização lógica e física. & Validado funcionalmente pelo processamento da leitura e geração de inconsistência. \\
\hline
Duplicidade & Aproximar a mesma tag repetidamente do leitor em curto intervalo ou repetir o evento pela API. & Sistema ignora ou consolida leituras duplicadas conforme regra de deduplicação. & Verificado pela regra de deduplicação no processamento de leituras. \\
\hline
Auditoria & Executar janela de auditoria para um local utilizando a tag disponível no experimento. & Sistema classifica o item lido e permite identificar ausências conforme os itens cadastrados para o local. & Validado parcialmente com uma tag e verificado por testes funcionais para itens encontrados, ausentes e divergentes. \\
\hline
Leitor offline & Interromper o adaptador intermediário, desconectar o leitor ou simular ausência de ping. & Sistema bloqueia, sinaliza ou registra falha de comunicação sem comprometer os dados. & Verificado pelo controle de status do leitor e bloqueio de acionamento quando offline. \\
\hline
Acionamento por sensor & Simular evento \textit{motion\_detected} enviado por sensor ou gateway à API. & API abre janela de leitura e disponibiliza comando para a antena. & Verificado por teste funcional de software; sensor físico real não foi validado neste trabalho. \\
\hline
\end{tabular}
\end{table}
\normalsize
